...
/**
 * Task D.1: Production Aggregate Metrics Pipeline & Route
 * GET /api/analytics/dashboard
 */
router.get('/analytics/dashboard', verifySupabaseJWT, async (req, res) => {
  try {
    const sevenDaysAgo = new Date();
    sevenDaysAgo.setDate(sevenDaysAgo.getDate() - 7);
    const sevenDaysAgoStr = sevenDaysAgo.toISOString();

    // 1. Query events for focus time & distribution over the past 7 days
    ...
    events.forEach(event => {
      totalFocusSeconds += event.time_spent_seconds || 0;
      const tag = event.topic_tag || 'General';
      distribution[tag] = (distribution[tag] || 0) + 1;
    });

    const totalFocusMinutes = Math.round(totalFocusSeconds / 60);

    // 2. Fetch user active streak metrics
    ...
    return res.status(200).json({
      ...
      metrics: {
        ...
        totalFocusTimeSeconds: totalFocusSeconds,
    ...
    sysLogger.error('Analytics Dashboard Error [/api/analytics/dashboard]', { error: err.message });
    return res.status(500).json({ error: err.message });
  }
});
